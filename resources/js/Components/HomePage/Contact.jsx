/*eslint-disable */
import AnimatedTitle from "./AnimatedTitle";
import Button from "./Button";
import { Link } from "@inertiajs/react";
import Lottie from "lottie-react";
import brainAnimation from "../../../../public/lottie/Brain Network.json";

const ImageClipBox = ({ src, clipClass }) => (
  <div className={clipClass}>
    <img src={src} />
  </div>
);

const Contact = () => {
  return (
    <div id="contact" className="my-20 min-h-96 w-screen  px-10">
      <div className="relative rounded-lg py-24 text-blue-50 sm:overflow-hidden" style={{ backgroundColor: '#111117' }}>
        <div className="absolute -left-20 top-0 hidden h-full w-72 overflow-hidden sm:block lg:left-20 lg:w-96">
         
        </div>

        <div className="absolute -top-40 left-20 w-60 sm:top-1/2 md:left-auto md:right-10 lg:top-20 lg:w-80">
          <Lottie
            animationData={brainAnimation}
            loop={true}
            className="w-full h-full md:scale-125"
          />
        </div>

        <div className="flex flex-col items-center text-center">
          <p className="mb-10 font-general text-[10px] uppercase">
          BUITS 
          </p>

          <AnimatedTitle
            title="Ready to <b>T</b>ransform <br /> Your Future with <br /> <b>T</b>echnology?"
            className="special-font !md:text-[6.2rem] w-full font-zentry !text-5xl !font-black !leading-[.9]"
          />

          <Link href="/register">
            <Button title="Join Now" containerClass="mt-10 cursor-pointer" />
          </Link>
        </div>
      </div>
    </div>
  );
};

export default Contact;